    ...
    in
    {
      checks = {
        ...
        cli-smoke = pkgs.runCommandLocal "cli-smoke" { } ''
          ...
          [ "$(${self'.packages.cli}/bin/portable-cli -c 'which kubectl | get type.0')" = "alias" ]
          [ "$(${self'.packages.cli}/bin/portable-cli -c 'which kubectl | get definition.0')" = "kubecolor" ]
          ...
          grep -Fqx 'alias "kubectl" = kubecolor' ${self'.packages.cli}/share/config.nu
          grep -Fqx 'alias "k" = kubecolor' ${self'.packages.cli}/share/config.nu
...
EOF
        ...
        host-override-smoke = pkgs.writeText "host-override-smoke" ''
          ...
          idle=${if overrideDesktop.config.repo.idle.lockSeconds == 300 && overrideDesktop.config.repo.idle.monitorOffSeconds == 360 then "300/360" else throw "idle override did not apply"}
          nightLight=${if overrideDesktop.config.repo.nightLight.dayTemperature == 6000 && overrideDesktop.config.repo.nightLight.nightTemperature == 3400 then "6000/3400" else throw "night light override did not apply"}
