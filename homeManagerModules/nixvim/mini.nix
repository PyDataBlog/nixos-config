{ lib, ... }:
{
  plugins.mini = {
    ...
    modules = {
      ...
      icons = {
        use_file_extension = lib.nixvim.mkRaw ''
          function(ext, _)
      ...
      snippets = {
        snippets = {
          "__unkeyed-1.from_file".__raw = ''
            (function()
              local paths = vim.api.nvim_get_runtime_file('snippets/global.json', true)
              local path = nil

              for _, candidate in ipairs(paths) do
                if candidate:match('/nvim%-config/') then
                  path = candidate
                  break
                end
              end
              ...
              return require('mini.snippets').gen_loader.from_file(path)
            end)()
          '';
          "__unkeyed-2.from_lang".__raw = ''
  ...
  extraConfigLuaPost = lib.mkAfter ''
    ...
    local start_codecompanion_spinner = function()
      ...
      codecompanion_spinner_timer:start(0, 120, vim.schedule_wrap(function()
